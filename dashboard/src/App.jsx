...
export default function App({ auth }) {
  ...
  const runFactoryAction = async (kind, machineKey, payload = {}) => {
    ...
    else if (kind === "missionStart") result = await startFactoryMission(machineKey, payload);
    else if (kind === "missionAction") result = await actOnFactoryMission(machineKey, payload);
    else if (kind === "pack") return downloadFactoryReadinessPack();
    await qc.invalidateQueries({ queryKey: ["factoryReadiness"] });
    if (["passport", "inventory", "probe", "missionStart", "missionAction"].includes(kind)) {
      qc.invalidateQueries({ queryKey: ["diagnostics"] });
    }
    return result;
  };
  ...
  const runPlanningAction = async (kind, data) => {
    ...
    else if (kind === "changeoverStandard") result = await updateChangeoverStandard(data.key, data.payload);
    else if (kind === "changeoverObservation") result = await recordChangeoverObservation(data);
    else if (kind === "changeoverExclude") result = await excludeChangeoverObservation(data.id, data.payload);
    else if (kind === "changeoverSync") result = await syncChangeovers(data);
    ...
    <div style={{ minHeight: "100vh", background: "#0d1117", color: "#f9fafb",
      ...
      {showCommissioning && (
        <CommissioningPanel machines={enriched} connectors={connectorSnapshot}
                            ...
                            onOpenSetup={() => { setShowCommissioning(false); setShowSetup(true); }}
