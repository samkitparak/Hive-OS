...
HIVE OS 0.33 can be installed and upgraded on a disconnected factory LAN. A
...
- `release\HIVE-OS-0.33.0-offline.zip`
- `release\HIVE-OS-0.33.0-offline.zip.sha256`
...
PYTHONPATH=src python src/offline_release.py \
  release/HIVE-OS-0.33.0-offline.zip --version 0.33.0
```
...
C:\HIVE-OS\deploy\windows\upgrade-hive.ps1 `
  -BundlePath D:\HIVE-OS-0.33.0-offline.zip
```

To upgrade a 0.20 installation, extract the 0.33 release and run the upgrader
included at its root; it carries the new backup implementation itself:

```powershell
D:\HIVE-OS-0.33.0-offline\upgrade-hive.ps1 `
  -BundlePath D:\HIVE-OS-0.33.0-offline
```
